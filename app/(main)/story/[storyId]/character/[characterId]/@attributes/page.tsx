...
export default function AttributesTab() {
  const [attributes, setAttributes] = useState(
    [] as { index: number; key: string; value: string }[],
  )

  const handleAddAttribute = () => {
    setAttributes([
      ...attributes,
      { index: attributes.length, key: '', value: '' },
    ])
  }
  ...
  const handleChangeAttribute = (
    ...
    setAttributes((prev) =>
      prev
        .map((attr) =>
          attr.index === index ? { ...attr, [field]: newValue } : attr,
        )
  ...
  return (
    <div className={style.attributesTab}>
      ...
      <div className={style.card}>
        ...
        <p className={style.addMore} onClick={handleAddAttribute}>
